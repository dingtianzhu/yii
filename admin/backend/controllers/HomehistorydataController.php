...
use backend\models\Homehistorydata;
...
class HomehistorydataController extends \yii\web\Controller
{
    ...
    public function actionList()
    {
        ...
        $data = Homehistorydata::find();
        $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '20']);
        $user = $data->joinWith('usergroup')->where("type=1")->offset($pages->offset)->limit($pages->limit)->all();
        return $this->render('list',[
            'user'=>$user,
            'pages' => $pages
        ]);
    ...
    public function actionList2()
    {
        $data = Homehistorydata::find();
        $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '20']);
        $user = $data->joinWith('usergroup')->where("type=2")->offset($pages->offset)->limit($pages->limit)->all();
    ...
    public function actionList3()
    {
        $data = Homehistorydata::find();
        $pages = new Pagination(['totalCount' =>$data->count(), 'pageSize' => '20']);
        $user = $data->joinWith('usergroup')->where("type=3")->offset($pages->offset)->limit($pages->limit)->all();
